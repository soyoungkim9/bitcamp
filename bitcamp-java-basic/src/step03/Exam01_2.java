...
public class Exam01_2 {
    public static void main(String[] args) {
        ...
        // => 식탁에 밥, 국, 반찬1, 반찬2, 반찬3을 장반에 담아서 가져오는 상황

        // 다양한 타입의 메모리를 묶어서 Score라는 새로운 형태의 메모리를
        // 만들 수 있는 설계도
        ...
        // 개발자가 새롭게 정의한 메모리 설계도에 따라 메모리르 준비한다.
        ...
        // => 다른 전문적인 용어로 "s가 가르키는 인스턴스의 name값"
...
// 클래스(Class)
// - 여러 타입을 묶어서 사용자정의 데이터타입을 만드는 문법
...
//
// primitive변수와 레퍼런스
// primitive type(byte, short, int, long, byte, double, boolean, char)의
// 메모리를 만들때 변수 선언 만으로 완료된다.
// - 변수 이름이 곧 메모리를 가리키는 이름이 된다.
//   예) int age; // 이자체가 메모리를 만듬 (변수선언만으로 완료)
// - class(사용자정의 데이터 타입)으로 메모리를 만들 때는
//   반드시 new 명령을 사용해야 한다.
// - 메모리를 만든 후에는 그 주소를 변수에 저장해서 메모리를 사용한다.
//   예) Score s; <--- 메모리의 주소를 저장하는 변수
//       s = new Score() <--- 메모리를 확보한 그 주소를 s에 저장
//
// 인스턴스의 항목 접근하기
// - 문법
//   레퍼런스명.항목명 = 값;
